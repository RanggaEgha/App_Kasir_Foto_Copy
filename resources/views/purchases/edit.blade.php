...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    ...
    <a href="{{ route('purchases.show', $po) }}" class="btn btn-sm btn-secondary">Lihat</a>
  </div>

  <div class="card-body">
    <form method="POST" action="{{ route('purchases.update', $po) }}">
      @csrf
      @method('PUT')
      ...
      <div class="row g-3">
        <div class="col-md-4">
          ...
          <select name="supplier_id" class="form-select" required>
            @foreach(($suppliers ?? []) as $s)
              <option value="{{ $s->id }}" @selected($po->supplier_id == $s->id)>{{ $s->name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="col-md-4">
          ...
          <input name="invoice_no" class="form-control" required value="{{ $po->invoice_no }}">
        </div>

        <div class="col-md-4">
          <label class="form-label">Tanggal</label>
          <input type="date" name="tanggal" class="form-control" value="{{ optional($po->tanggal)->format('Y-m-d') }}">
        </div>

        <div class="col-md-4">
          <label class="form-label">Metode Bayar</label>
          <select name="metode_bayar" class="form-select">
            <option value="tunai"    @selected($po->metode_bayar === 'tunai')>Tunai</option>
            <option value="transfer" @selected($po->metode_bayar === 'transfer')>Transfer</option>
            <option value="tempo"    @selected($po->metode_bayar === 'tempo')>Tempo</option>
          </select>
        </div>
        ...
        <div class="col-md-4">
          ...
          <input name="discount" class="form-control" placeholder="cth: 100.000"
                 value="{{ $po->discount == 0 ? '' : number_format($po->discount,0,',','.') }}">
        </div>

        <div class="col-md-4">
          <label class="form-label">PPN (%)</label>
          <input name="tax_percent" class="form-control" placeholder="cth: 11"
                 value="{{ rtrim(rtrim(number_format($po->tax_percent,2,',','.'),'0'),',') }}">
        </div>
      </div>
          ...
          <tbody>
            @forelse($po->items as $i => $it)
              <tr>
                ...
                <td>
                  <select name="items[{{ $i }}][barang_id]" class="form-select" required>
                    @foreach(($barangs ?? []) as $b)
                      <option value="{{ $b->id }}" @selected($it->barang_id == $b->id)>{{ $b->nama }}</option>
                    @endforeach
                  </select>
                </td>

                <td>
                  <select name="items[{{ $i }}][unit_id]" class="form-select" required>
                    @foreach(($units ?? []) as $u)
                      <option value="{{ $u->id }}" @selected($it->unit_id == $u->id)>{{ $u->kode }}</option>
                    @endforeach
                  </select>
                </td>

                <td>
                  <input name="items[{{ $i }}][qty]" type="number" min="1" class="form-control text-end"
                         value="{{ $it->qty }}">
                </td>

                <td>
                  <input name="items[{{ $i }}][price]" class="form-control text-end"
                         placeholder="cth: 5.000.000"
                         value="{{ number_format($it->unit_price,0,',','.') }}">
                </td>
            ...
            @empty
              <tr>
                ...
                <td><input name="items[0][price]" class="form-control text-end" placeholder="cth: 30.000"></td>
                <td class="text-end">
                  <button type="button" class="btn btn-sm btn-outline-danger btn-remove">Hapus</button>
                </td>
              </tr>
